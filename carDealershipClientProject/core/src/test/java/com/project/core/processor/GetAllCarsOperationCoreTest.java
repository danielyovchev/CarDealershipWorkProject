...
class GetAllCarsOperationCoreTest {
    @Mock
    private GetAllCars getAllCars;
    @Mock
    private DomainToResponseMapper mapper;
    @InjectMocks
    private GetAllCarsOperationCore getAllCarsOperationCore;
    @BeforeEach
    void setUp() {
    ...
    void process() {
        ...
        CarListDomainResponse carListDomainResponse =   CarListDomainResponse
                ...
                .build();
        Mockito.when(getAllCars.getAllCars()).thenReturn(carListDomainResponse);
        AllCarRequest allCarRequest = new AllCarRequest();
        assertEquals(getAllCarsOperationCore.process(allCarRequest).get().getClass(), CarListResponse.class);
    }
}